...
def find_closest(location, centroids):
    ...
    """
    # BEGIN Question 3
    return min(centroids, key=lambda c : distance(c, location))
    # END Question 3
...
def group_by_centroid(restaurants, centroids):
    ...
    for rest in restaurants:
        loc = restaurant_location(rest)
        nearest_centroid = find_closest(loc, centroids)
        pair = [nearest_centroid, rest]
        pairs.append(pair)
...
def find_centroid(cluster):
    ...
    # BEGIN Question 5
    xs = [restaurant_location(loc)[0] for loc in cluster]
    ys = [restaurant_location(loc)[1] for loc in cluster]
...
def best_predictor(user, restaurants, feature_fns):
    ...
    # BEGIN Question 8
    # Use max function with key and list generator and list converter
    return max([list(find_predictor(user, reviewed, fn)) for fn\
            in feature_fns], key = lambda p : p[1])[0]
