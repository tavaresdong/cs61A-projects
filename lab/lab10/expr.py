...
class LambdaExpr(Expr):
    ...
    LambdaExpr(['x', 'y'], CallExpr('add', [Name('x'), Name('y')]))
...
class Value:
    ...
    Lambda expressions have no intrisic parent environment (they are simply
    pieces of code), while lambda functions remember the environment in which
    they were created. The same lambda expression may create different lambda
    expressions with different parent environments.

    Consider this (actual Python) example:

    def make_adder(x):
        return lambda y: x + y

    There is one lambda expression in the code above (lambda y: x + y). However,
    if we were to call make_adder(3) and make_adder(4), we would create two
    different lambda functions with two different parent environments.
    ...
    Attempting to apply a `Number` (e.g. as in 4(2, 3)) will raise an exception.
